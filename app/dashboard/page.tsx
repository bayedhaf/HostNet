...
export default function UserDashboard() {
  ...
  const filters = ["Hotel Waiter", "Cafe waiter", "Restaurant waiter", "Kitchen", "All works"];

  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const res = await fetch("https://dummyjson.com/c/fa6c-5a29-4ac1-a859");///api/application
        if (!res.ok) throw new Error("Failed to fetch applications");
  const data: Application[] = await res.json();
        setApplications(data || []);
      } catch {
        setError(language === "Oromo" ? "Hojjettota arguu hin dandeenye" : "Failed to fetch employees");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    const matchesFilter =
      selectedFilter === "All works" || app.about.toLowerCase().includes(selectedFilter.toLowerCase());
          ...
          {filters.map((f, idx) => (
            <Button
              ...
              className={`rounded-full px-4 py-2 ${
                selectedFilter === f ? "bg-cyan-500 text-white" : "bg-cyan-100 hover:bg-cyan-200"
              }`}
